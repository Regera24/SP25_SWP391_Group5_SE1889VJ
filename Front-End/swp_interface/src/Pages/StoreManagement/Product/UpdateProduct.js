...
const UpdateProduct = ({ product, onClose, fetchProducts }) => {
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      categoryID: product.categoryID, // Giả sử product chứa categoryID
      storeDetailProductAttributeDTOList: product.storeDetailProductAttributeDTOList
        ? product.storeDetailProductAttributeDTOList.map(attr => attr.id)
        : [],
      storeDetailZoneDTOList: product.storeDetailZoneDTOList
        ? product.storeDetailZoneDTOList.map(zone => zone.id)
        : [],
    });
  ...
  const handleUpdateProduct = async (values) => {
    ...
    try {
      ...
      const updatedProductData = {
        ...
        storeDetailProductAttributeDTOList: values.storeDetailProductAttributeDTOList ? values.storeDetailProductAttributeDTOList.map(id => ({ id })) : [],
        storeDetailZoneDTOList: values.storeDetailZoneDTOList ? values.storeDetailZoneDTOList.map(id => ({id})) : [],
      };
  ...
  return (
    <Form form={form} layout="vertical" onFinish={handleUpdateProduct}>
      ...
      <Form.Item
        ...
        name="storeDetailProductAttributeDTOList"
      >
        <Select mode="multiple" placeholder="Chọn thuộc tính của sản phẩm">
          {productAttributes.map((attribute) => (
            <Select.Option key={attribute.value} value={attribute.id}>
              {attribute.value}
            </Select.Option>
          ))}
        </Select>
      ...
      >
        <Select mode='multiple' placeholder="Chọn danh sách khu vực">
          {zones.map((zone) => (
            <Select.Option key={zone.name} value={zone.id}>
              {zone.name}
            </Select.Option>
          ))}
        </Select>
